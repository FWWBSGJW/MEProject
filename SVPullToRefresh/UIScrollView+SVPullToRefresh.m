...
// public properties
@synthesize pullToRefreshActionHandler, arrowColor, textColor, activityIndicatorViewStyle, lastUpdatedDate, dateFormatter, beforeTime, currentTime;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        //self.textColor = [UIColor colorWithRed:153 green:153 blue:153 alpha:1.0]; //RGBCOLOR(153, 153, 153);//[UIColor darkGrayColor];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        if(currentTime == 0)
        {
            currentTime = (long)[[NSDate date] timeIntervalSince1970];
        }
        beforeTime = (long)[[NSDate date] timeIntervalSince1970];
        //if(currentTime != beforeTime)
        {
            timeStr = [NSString stringWithFormat:@"最后更新时间:%@",[DataOC getTimeString:currentTime]];
        }
        self.subtitleLabel.text = timeStr;//subtitle.length > 0 ? subtitle : nil;
        ...
        CGFloat offsetx = (subtitleSize.width-titleSize.width)/2;
        ...
        if(subtitleSize.height > 0){
            ...
            CGFloat titleY = minY;
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX+offsetx, titleY, titleSize.width, titleSize.height));
...
//	CGContextAddRect(c, CGRectMake(5, 12, 12, 4));
	...
	CGContextAddRect(c, CGRectMake(7, 18, 8, 4));
//	CGContextAddRect(c, CGRectMake(5, 30, 12, 4));
	// the arrow
...
//	CGContextClosePath(c);
 	...
 	CGContextMoveToPoint(c, 2, 22);
	CGContextAddLineToPoint(c, 11, 36);
	CGContextAddLineToPoint(c, 20, 22);
	CGContextAddLineToPoint(c, 3, 22);
	CGContextClosePath(c);
    ...
    if([[[UIDevice currentDevice] systemVersion]floatValue] >= 5){
        NSArray* alphaGradientColors = [NSArray arrayWithObjects:
                                        ...
                                        (id)[self.arrowColor colorWithAlphaComponent:0.6].CGColor,
                                        nil];
